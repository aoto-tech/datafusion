...
If rewriting is impractical, set
`datafusion.execution.parquet.infer_legacy_null_counts` to `true`. DataFusion
will then treat missing counts as zero for files whose `created_by` identifies
parquet-rs before 53.1.0 or DataFusion before 42.1.0. This is opt-in because
`created_by` is configurable; enable it only when those files have not had their
statistics removed or replaced by another tool.
