...
| INFER_LEGACY_NULL_COUNTS                   | No                      | If true, treats missing null counts as zero when `created_by` identifies parquet-rs before 53.1.0 or DataFusion before 42.1.0. This is opt-in because `created_by` is configurable and is not proof that a missing count is zero.                                                                                                    | `'infer_legacy_null_counts'`                          | false                    |
